...
class __Tag_csrf_token extends AbstractTag{
	...
	public static function getHash(){
		if(self::$hash==null){
			...
			//clean up old tokens
			$header=md5("csrftoken");
			foreach($_SESSION as $k=>$v){
				if(strlen($v)>=strlen($header)&&substr($v,0,strlen($header))==$header)
					unset($_SESSION[$k]);
			}
			
			//note that strlen($buffer) may be < strlen($s1) + strlen($s2)
			self::$hash=$header.substr(str_shuffle(str_repeat($buffer,7)),0,strlen($buffer)/2); //shuffles everything around
...
class __Tag_extends extends AbstractTag{
	public static function render($args,$c,&$vars){
		...
		$blocks=array();
		$parent=$c;
		while(!($parent instanceof RootComponent))
			$parent=$parent->parent;
		$queue=$parent->components;
		...
		while(count($queue)){
			...
			if($comp instanceof TagComponent){
				if($comp->getName()=="extends"){
					//do nothing if it's the extends block, otherwise we run into an ininite loop
				}
